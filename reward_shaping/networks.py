...
class BasicActorREINFORCE(nn.Module):
    def __init__(self, observation_space, action_space):
        super().__init__()
        self.fc1 = nn.Linear(np.array(observation_space.shape).prod(), 256)
        self.fc2 = nn.Linear(256, 256)
        self.fc_mean = nn.Linear(256, np.prod(action_space.shape))
        self.fc_std = nn.Linear(256, np.prod(action_space.shape))
        ...
        self.fc_std.weight.data.fill_(0.5)
        self.fc_std.bias.data.fill_(0.0)
    ...
    def forward(self, x, temperature=1.0):
        ...
        std = F.softplus(log_std) + 1.0  
        std = std * temperature  

        mean = torch.tanh(mean) * self.action_scale + self.action_bias
        return mean, std

    def get_action(self, x, action=None, temperature=1.0):
        mean, std = self(x, temperature)
...
class BasicActorDDPG(nn.Module):
    def __init__(self, observation_space, action_space, block_num=3):
        ...
        self.fc_action = nn.Linear(128, np.prod(action_space.shape))
...
class BasicCriticA3C(nn.Module):
